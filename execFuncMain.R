...
tool_exec <- function(in_params, out_params) {
  ...
  recruitmentTimeSpan <- in_params[[6]]
  
  #*****Planning Input 
  planMeetingsCountry <- in_params[[7]]
  planMeetingsCity <- in_params[[8]]
  planTrainingCountry <- in_params[[9]]
  planTrainingCity <- in_params[[10]]
  planningTimeSpan <- in_params[[11]]  
  
  ##****Preparatory Input
  prepWeapons <- in_params[[12]]
  prepFunds <- in_params[[13]]
  prepIllegalImmigration <- in_params[[14]]
  prepSurveillance <- in_params[[15]]
  prepTimeSpan <- in_params[[16]]
 ...
 DAPredictingUsercase <- out_params[[2]]
  ...
  td <- table(clusteredData$cluster, testData$Terror_Event)   #OUTPUT 1      
  td
  summary(td)
  ...
  ##Sednign ouput 3
  t2 <- table(cl_predict(clusteredData, T2))    
  t2
  summary(t2)
  
  t2 <- as.data.frame(t2)
  
  if (!is.null(KMEANSpredictinguserCase) && KMEANSpredictinguserCase != 'NA') {
    arc.write(KMEANSpredictinguserCase, t2)
  }
